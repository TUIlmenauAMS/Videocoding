...
for n in range(25):
    ...
    if ret==True:
        ...
        Crfilt=np.clip(scipy.signal.convolve2d(Cr,filt2,mode='same'),-127,127)
        Cbfilt=np.clip(scipy.signal.convolve2d(Cb,filt2,mode='same'),-127,127)
 ...
 #Motion estimation, correlate current Y block with previous 16x16 block which contains current 8x8 block:
          ...
          for yblock in range(1,rows//8-1):
             #print("yblock=",yblock)
             #block[0]=yblock*8+200;
             for xblock in range(1,columns//8-1):
                #print("xblock=",xblock)
                #block[1]=xblock*8+300;
                #print("block= ", block)
                #current block:
                Yc=Y[yblock*8:yblock*8+8 ,xblock*8:xblock*8+8]
                #print("Yc= ",Yc)
                #previous block:
                ...
                for ymv in range(-8,8):
                   for xmv in range(-8,8):
                      diff=Yc-Yprev[yblock*8+ymv : yblock*8+ymv+8, xblock*8+xmv: xblock*8+xmv+8];
                ...
                #print(xblock, yblock)
                #print(mv[yblock,
                cv2.line(framevectors, (xblock*8, yblock*8), (xblock*8+mv[yblock,xblock,1].astype(int),yblock*8+mv[yblock,xblock,0].astype(int)) , (1.0,1.0,1.0));
          Yprev=Y.copy();
          print("store motion vectors:")
          MV=np.array(mv, dtype='int8')
          pickle.dump(MV,g)
